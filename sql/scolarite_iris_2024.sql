...
create procedure deleteClasse (IN p_idclasse int) 
	begin
	#suppression de tous les enseignements liés à cette classe 
	delete from enseignement where idclasse = p_idclasse;
 
	# suppression de tous les etudiants liés à cette classe 
 ...
 drop procedure if exists insertClasse ;
  ...
  declare nb int ;
      ...
      set p_salle ="Salle 3.10";
       ...
       select count(*) into nb from classe  where nom = p_nom ;
       if nb = 0 then


       insert into classe values (null, p_nom, p_salle, p_diplome, p_diplome);
       	end if ;
   end $
delimiter  ;
